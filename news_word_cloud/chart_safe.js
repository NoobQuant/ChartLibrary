...
var margin = {top: 0, right: 400, bottom: 70, left: 0};
var sBarMinWidth = 758
var sBarWidth = 310

var widthChart
widthChart = parseInt(d3.select('body').style('width'), 10) - margin.right

var heightChart = 800 - margin.top - margin.bottom;
...
function restartNodes(n_nodes,svg,rem_words_array, crt_week) {
  ...
  d3.json(input_json, function(error, json) {  
      ...
      if (error){
        ...
        if(error.statusText === "File not found") {
  
        // Should be put to inform user of week not available....                  
...
function plus_node(){
   ...
   restartNodes(+document.getElementById("nNodes").value,svg,dataset,week_number)
}
...
// Source: http://bl.ocks.org/eesur/9910343
...
function remove_common_words(){
    ...
    d3.json("news_word_cloud/typical_finnish_words.json", function(data) {
      var word_bulk = ""
      data.forEach(function(crt_word) {
        if (word_bulk === ""){
